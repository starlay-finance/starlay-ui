...
export type GetCurrentPriceQuery = {
  ...
  priceCurrent?: {
    ...
    data: number
    bottomPrice: number
    amountOfRaised: number
    boostedAmountOfRaised: number
    timestamp: number
  } | null
}
...
export type ListPricesHistoricalQuery = {
  __typename?: 'Query'
  prices5Min: {
    __typename?: 'PricesOutput'
    items: Array<{
...
export function getSdk(
  ...
  return {
    GetCurrentPrice(
      variables: GetCurrentPriceQueryVariables,
      requestHeaders?: Dom.RequestInit['headers'],
    ): Promise<GetCurrentPriceQuery> {
      return withWrapper(
        (wrappedRequestHeaders) =>
          client.request<GetCurrentPriceQuery>(
            GetCurrentPriceDocument,
            variables,
    ...
    ListPricesHistorical(
      variables: ListPricesHistoricalQueryVariables,
      requestHeaders?: Dom.RequestInit['headers'],
    ): Promise<ListPricesHistoricalQuery> {
      return withWrapper(
        (wrappedRequestHeaders) =>
          client.request<ListPricesHistoricalQuery>(
            ListPricesHistoricalDocument,
            variables,
            { ...requestHeaders, ...wrappedRequestHeaders },
          ),
        'ListPricesHistorical',
